<script lang="ts" context="module">
  // Modified from: https://svelte.dev/repl/bfbbf8db4c994bc6962ac17d7e8b3e11?version=3.44.2
...
<script lang="ts">
  ...
  function confettiStyles(node, { degree }) {
    ...
    const y4 = round(
      Math.max(mapRange(Math.abs(degree - 180), 0, 180, force, -force), 0),
      4
    );
    ...
    setCSSVar(
      ...
      `${
        isCircle
          ? particleSize
          : Math.round(Math.random() * 4) + particleSize / 2
      }px`
    );
    setCSSVar(
      "--height",
      (isCircle ? particleSize : Math.round(Math.random() * 2) + particleSize) +
        "px"
    );
